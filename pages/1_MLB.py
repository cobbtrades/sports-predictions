...
team_name_mapping = {'ARI': 'Arizona Diamondbacks','ATL': 'Atlanta Braves','AZ': 'Arizona Diamondbacks','BAL': 'Baltimore Orioles','BOS': 'Boston Red Sox','CHC': 'Chicago Cubs','CHW': 'Chicago White Sox',
...
if error:
    ...
    data = {'Tm': [g['home_team_abbr'] for g in games],'Opp': [g['away_team_abbr'] for g in games],'TmStart': [g['TmStart'] for g in games],
        'OppStart': [g['OppStart'] for g in games],'Tm_ml': [g['home_ml'] for g in games],'Opp_ml': [g['away_ml'] for g in games]}
...
display_df.rename(columns={'Tm': 'Home Team','Opp': 'Away Team','Tm_ml': 'Home Odds','Opp_ml': 'Away Odds','Predicted Winner': 'Predicted Winner','Win Probability': 'Win Probability','TmStart': 'Home Pitcher','OppStart': 'Away Pitcher'}, inplace=True)
...
team_colors = {'Matchup': '#ffaf42', 'Predicted Winner': '#49f770', 'Winner Odds': '	#2daefd'}
header_style = [{
    'selector': 'th',
...
def colorize_column(s):
    color = team_colors.get(s.name, 'white')
    return [f'color: {color}; background-color: black; text-align: center' for v in s]
styled_df = final_display_df.style.apply(colorize_column)\
